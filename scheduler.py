...
Runs two recurring jobs:
  ...
  2. check_replies()      — every 5m  — scans IMAP for replies, pauses sequences
     ...
     running, and the Settings toggle exists for exactly that case. A manual
...
Nothing WhatsApp-related runs in this loop. Sending is manual, the follow-up
cadence is a live query (db.get_wa_followups_due), and the automatic website
check that used to run here was retired: a lead goes out straight from its
scrape, and the operator audits it when they choose (audit.py, on a click).
"""
