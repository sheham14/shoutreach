...
Contacts is email_leads joined to businesses now (see the contacts->businesses
split); this file exercises that view rather than a bare `contacts` table.
"""
import importlib
import os
import shutil
...
def main():
    ...
    try:
        ...
        db.upsert_businesses([{"email": "manual@byhand.ca", "company": "By Hand"}])

        page = db.get_email_leads_page(page=1, per_page=50)
        ...
        first = db.get_email_leads_page(page=1, per_page=50, source_job_id=job_a)
        ...
        manual = db.get_email_leads_page(page=1, per_page=50, source_job_id=db.SOURCE_MANUAL)
        ...
        print("\n2. QUALIFYING FIELDS ARE REAL COLUMNS ON THE BUSINESS, NOT `extra`")
        row = first["rows"][0]
        ...
        with db.get_db() as conn:
            extra = conn.execute(
                "SELECT extra FROM businesses WHERE id=?", (row["business_id"],)
            ).fetchone()["extra"]
        check("extra left empty", extra in ("{}", ""), f"extra={extra}")

        print("\n3. PAGING DOES NOT LOSE OR REPEAT ROWS")
        p1 = db.get_email_leads_page(page=1, per_page=30)
        p2 = db.get_email_leads_page(page=2, per_page=30)
        p3 = db.get_email_leads_page(page=3, per_page=30)
        ...
        hit = db.get_email_leads_page(page=1, per_page=10, q="clinic57")
        ...
        by_phone = db.get_email_leads_page(page=1, per_page=10, q="555-0003")
        check("search covers phone too", by_phone["total"] >= 1, f"total={by_phone['total']}")

        combined = db.get_email_leads_page(page=1, per_page=10, q="pipes", source_job_id=job_a)
        check("search and list filter combine (AND, not OR)", combined["total"] == 0,
              f"total={combined['total']}")

        print("\n5. SORTING IS WHITELISTED")
        asc = db.get_email_leads_page(page=1, per_page=5, sort_col="company", sort_dir="asc")
        desc = db.get_email_leads_page(page=1, per_page=5, sort_col="company", sort_dir="desc")
        ...
        bad = db.get_email_leads_page(page=1, per_page=5, sort_col="company; DROP TABLE businesses")
        check("an unknown sort column is ignored, not interpolated", bad["sort_col"] == "")
        with db.get_db() as conn:
            still_there = conn.execute("SELECT COUNT(*) FROM email_leads").fetchone()[0]
        check("email_leads table survived the injection attempt", still_there == 81)
        ...
        check("every selected lead is gone",
              db.get_email_leads_page(page=1, per_page=100, source_job_id=job_b)["total"] == 0)
        check("the hidden soft-deleted row was left alone, not swept up",
              db.get_email_leads_page(page=1, per_page=100, source_job_id=job_b,
                                      include_deleted=True)["total"] == 1)
        check("the other list is untouched",
              db.get_email_leads_page(page=1, per_page=100, source_job_id=job_a)["total"] == 60)
