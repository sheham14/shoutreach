...
enrollment queued on a step the address already received, so the scheduler
...
country. Identity is a row in `businesses` now, found by find_existing_business
or find_or_create_business regardless of which of those fields a given row
happens to carry.
"""
import importlib
import os
...
def main():
    ...
    try:
        ...
        db.upsert_businesses([{"email": "a@guard1.ca", "company": "Guard 1",
                               "website": "https://guard1.ca"}])
        lead_id = db.get_email_lead_by_email("a@guard1.ca")["id"]

        check("nothing sent yet", db.has_sent_step(cid, lead_id, 1) is False)
        db.log_send(cid, lead_id, 1, "Subject", "msg-1")
        check("the delivered step is now recognised",
              db.has_sent_step(cid, lead_id, 1) is True)
        check("a later step is not", db.has_sent_step(cid, lead_id, 2) is False)
        check("nor the same step in another campaign",
              db.has_sent_step(cid + 999, lead_id, 1) is False)
        ...
        ids = [db.get_email_lead_by_email(f"b{i}@small{i}.ca")["id"] for i in range(12)]
        db.upsert_step(small, 1, "s", "b", 0)
        db.enroll_contacts_bulk(small, ids)
        for n, lead_id2 in enumerate(ids):
            db.log_send(small, lead_id2, 1, "s", f"sm{n}")
        with db.get_db() as conn:
            conn.execute("UPDATE enrollments SET status='bounced' WHERE campaign_id=? "
                         "AND email_lead_id=? ", (small, ids[0]))
        ...
        more = [db.get_email_lead_by_email(f"c{i}@big{i}.ca")["id"] for i in range(30)]
        db.enroll_contacts_bulk(small, more)
        for n, lead_id2 in enumerate(more):
            db.log_send(small, lead_id2, 1, "s", f"bg{n}")
        with db.get_db() as conn:
            conn.execute("UPDATE enrollments SET status='bounced' WHERE campaign_id=? "
                         "AND email_lead_id IN ({})".format(",".join("?" * 10)),
                         (small, *more[:10]))
        should, rate, sends = db.bounce_breaker_should_pause(small, 5.0)
        ...
        print("\n6. NORMALIZED PHONE IS STORED ALONGSIDE THE DISPLAY VALUE")
        with db.get_db() as conn:
            row = conn.execute(
                "SELECT phone, phone_normalized FROM businesses WHERE id=?", (clinicx_id,)
            ).fetchone()
        ...
        # No separate backfill pass exists for businesses -- it's a new table,
        # so every row is written by code that already computes
        # phone_normalized. What still needs covering is the merge behaviour:
        # a re-import must fill a blank rather than leaving it blank forever.
        with db.get_db() as conn:
            conn.execute("UPDATE businesses SET phone_normalized='' WHERE id=?", (clinicx_id,))
        db.upsert_businesses([{
            "email": "hi@clinicx.ca", "website": "https://clinicx.ca",
            "phone": "(709) 555-7777",
        }])
        with db.get_db() as conn:
            refilled = conn.execute(
                "SELECT phone_normalized FROM businesses WHERE id=?", (clinicx_id,)
            ).fetchone()["phone_normalized"]
        check("a re-import refills a blanked phone_normalized",
              refilled == "7095557777", refilled)

        print("\n7. TOUCH HISTORY REPORTS RATHER THAN HIDES")
        bounced_business_id = db.get_email_lead_by_email(f"b0@small0.ca")["business_id"]
        hist = db.get_touch_history(bounced_business_id)
        ...
        untouched = db.get_touch_history(clinicx_id)
